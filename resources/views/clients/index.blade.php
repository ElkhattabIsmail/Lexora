                ...
                <div class="p-4 border-b border-slate-100">
                    <form method="GET" action="{{ route('clients.index') }}" class="space-y-3">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            @endif
                            <span class="ml-auto text-sm text-slate-500 shrink-0">{{ $clients->total() }} client(s)</span>
                        </div>
                    </form>
                </div>
                ...
                <div class="md:hidden divide-y divide-slate-100">
                    @forelse ($clients as $client)
                        <div class="p-4 hover:bg-slate-50 transition-colors">
                            ...
                            <div class="flex items-center gap-3 mt-2 ml-14">
                                @if ($client->isEntreprise())
                                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-brand-50 text-brand-DEFAULT">Entreprise</span>
                                @else
                                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-700">Particulier</span>
                                @endif
                                @if ($client->telephone)
                                    <span class="text-xs text-slate-500">{{ $client->telephone }}</span>
                                @endif
                                <span class="text-xs text-slate-500 ml-auto">{{ $client->dossiers_count }} dossier(s)</span>
                            </div>
                        </div>
                    @empty
