<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-serif font-bold text-2xl text-slate-800 leading-tight">Nouvelle Facture</h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('factures.store') }}" method="POST">
                    ...
                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="client_id" name="client_id" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-DEFAULT focus:ring focus:ring-brand-DEFAULT/20 @error('client_id') border-red-500 @enderror">
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('client_id') == $client->id ? 'selected' : '' }}>{{ $client->nom_complet }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" id="montant" name="montant" value="{{ old('montant') }}" min="0" step="0.01" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-DEFAULT focus:ring focus:ring-brand-DEFAULT/20 @error('montant') border-red-500 @enderror" placeholder="0.00">
                        ...
                        <div>
                            ...
                            <input type="date" id="date_facture" name="date_facture" value="{{ old('date_facture', now()->format('Y-m-d')) }}" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-DEFAULT focus:ring focus:ring-brand-DEFAULT/20 @error('date_facture') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <select id="statut" name="statut" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-brand-DEFAULT focus:ring focus:ring-brand-DEFAULT/20">
                                @foreach (['Non payée', 'Payée'] as $s)
                                    <option value="{{ $s }}" {{ old('statut', 'Non payée') === $s ? 'selected' : '' }}>{{ $s }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="px-4 sm:px-8 py-5 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3 rounded-b-2xl">
                        <a href="{{ route('factures.index') }}" class="btn-secondary">Annuler</a>
